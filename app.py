...
st.markdown("""
    ...
    /* 🚨 가로 팽창의 주범이었던 줄바꿈 금지 제거 및 허용(pre-wrap) 처리 */
    .stButton>button {
        height: auto !important; 
        min-height: 45px !important; 
        white-space: pre-wrap !important; /* 글자가 길면 자연스럽게 줄바꿈됨 */
    ...
    /* 🔥 모든 탭(작업, 건강, 관리자) 강제 균등 분할 및 화면 밖 밀림 차단 */
    @media (max-width: 768px) {
        ...
        div[data-testid="column"] {
            ...
            min-width: 0 !important;
            width: 0 !important; /* 버튼 글씨가 길어도 칼같이 N등분 크기 유지 */
        }
    }
    </style>
...
if st.session_state.admin_mode:
    ...
    with tab1:
        ...
        for task_id, t_info in list(st.session_state.tasks.items()):
            ...
            # 글자 수 방어막 (5글자 이상이면 자름)
            display_title = t_info['업무세부분류']
            if len(display_title) > 5:
                display_title = display_title[:5] + ".."
            ...
            with c2:
                # \n 을 넣어 아이콘과 시간을 위아래로 분리!
                if t_info['status'] == 'running':
                    if st.button(f"⏸️\n{time_display}", key=f"p_{task_id}", use_container_width=True): 
                        pause_task(task_id)
                        st.rerun()
                else:
                    if st.button(f"▶️\n{time_display}", key=f"r_{task_id}", use_container_width=True): 
                        resume_task(task_id)
                        st.rerun()
            with c3:
    ...
    with tab4:
        ...
        if balance_mins < 0:
            ...
            st.success(f"✨ **여유 시간: {balance_mins}분** (업무 노동: {spent_work_time}분)")
