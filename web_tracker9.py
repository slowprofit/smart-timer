...
def init_google_sheets():
    # secrets.json 열쇠를 사용해 구글 클라우드에 로그인
    gc = gspread.service_account(filename='secrets.json')
    # 공유해둔 'time_management' 파일 열기
    sh = gc.open('time_management')
    return sh

sh = init_google_sheets()

# 시트가 없으면 자동으로 만들어주는 도우미 함수
...
def load_all_config():
    ...
    if records:
        cats = [str(r['Value']) for r in records if r['Category'] == 'Cats']
        if not cats: cats = ['카페', '온라인', '개인', '집안일']
...
def end_task(task_id):
    ...
    # 구글 시트 'Tasks' 탭에 새로운 줄 추가 (데이터 전송)
    ws_tasks.append_row([
...
# --- 🚀 건강 버튼 클릭 시 구글 시트에 즉시 기록 ---
def log_health(item_name, earned_minutes):
    now = datetime.now()
    # 구글 시트 'Health' 탭에 데이터 전송
...
# ==========================================
    ...
    with tab4:
        ...
        if not tdf.empty:
            ...
            if not tdf_today.empty: spent_time = int(tdf_today['실제 일한 시간(분)'].sum())

        if spent_time > earned_time: st.error(f"🚨 시간 부족! 확보({earned_time}분) < 사용({spent_time}분)")
        else: st.success(f"✨ 밸런스 굿! 잔여 시간 **{earned_time - spent_time}분**")
        
        st.bar_chart(pd.DataFrame({'지표': ['경과', '운동', '업무'], '시간(분)': [elapsed_mins, earned_time, spent_time]}).set_index('지표'), height=150)

        hourly_df = pd.DataFrame(index=[f"{h:02d}시" for h in range(7, 24)], columns=['운동', '업무']).fillna(0)
